...
#   ./deploy/run_policy.sh --keep-going          # do not halt on a STOP token
#   ./deploy/run_policy.sh --prompt "Walk down the corridor."   # start without waiting
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --keep-going) EXTRA+=("stop_behavior:=continue"); shift ;;
    --prompt) EXTRA+=("autostart:=true" "default_prompt:=$2"); shift 2 ;;
