...
```bash
./deploy/docker/run_server.sh                     # serves 0.0.0.0:5801
python3 deploy/docker/smoke_test.py               # dummy frame -> action list
```

**3. Build and run the node**

```bash
cd deploy/ros2_ws && colcon build && source install/setup.bash
ros2 launch streamvln_policy streamvln_policy.launch.py host:=<server-ip>
```
...
Hold the deadman button and the robot drives. Watch `/streamvln/status` for state,
latency and errors.
...
- **Speed follows from `chunk_seconds`.** StreamVLN's tokens are 0.25 m and 15°, so a 1 s
  chunk of 4 tokens demands 1.0 m/s and 60 °/s. That is brisk indoors — raise
  `chunk_seconds` to slow down while keeping the geometry exact. Lowering `v_max` instead
  makes the robot under-travel relative to what the model believes it did.
...
| control loop | 1 request/s at `chunk_seconds: 1.0`, **96 % motion duty cycle** |
